...
def img2tensor(img, bgr2rgb=True, float32=True):
    """HWC uint8/float numpy image (BGR from cv2) -> CHW torch tensor (same semantics as basicsr.utils.img2tensor)."""
    ...
    return tensor.float() if float32 else tensor
...
def main():
    ...
    # Initialize logger (log file name follows the first result directory)
    if args.log_name == 'METRICS':
        args.log_name = os.path.basename(os.path.normpath(args.inp_imgs[0])) or 'METRICS'
