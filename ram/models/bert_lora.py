...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def init_weights(self):
        """Plain weight initialisation without transformers' weight-tying machinery.

        transformers >= 5 reworked ``init_weights()`` / ``tie_weights()`` and they no longer
        work for this stand-alone BERT; the RAM checkpoint is loaded afterwards anyway.
        """
        self.apply(self._init_weights)

    # --- helpers that transformers 4.x provided through ModuleUtilsMixin and 5.x removed ---
    @property
    def dtype(self):
